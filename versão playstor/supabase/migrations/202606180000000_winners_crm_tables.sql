-- Migration: 202606180000000_winners_crm_tables.sql
-- Description: Create core schema for Winners CRM SaaS with multi-tenant structures with text-based primary/foreign keys

-- Enable uuid-ossp extension (kept for general UUID utility use)
create extension if not exists "uuid-ossp";

-- Drop existing tables for clean migration run
...
-- 1. Empresas (Tenants supporting slug and UUID strings)
...
-- 2. Utilizadores (Users linked by text enterprise identifier)
...
create index if not exists idx_utilizadores_empresa_id on public.utilizadores(empresa_id);

-- 3. Clientes (Client Profiles representing CRM consumers)
...
-- 4. Financeiro Metricas (Financial & Acquisition Metrics per Empresa)
create table if not exists public.financeiro_metricas (
...
-- 5. Componentes de Hambúrguer (Custom ingredients / products)
create table if not exists public.burger_components (
    id text not null, -- raw id string
    empresa_id text not null references public.empresas(id) on delete cascade,
    name text not null,
    category text not null, -- 'bread' | 'meat' | 'cheese' | 'sauce' | 'extra'
...
create index if not exists idx_burger_components_empresa_id on public.burger_components(empresa_id);

-- 6. Produtos do Cardápio Digital (Digital menu items)
create table if not exists public.produtos_menu (
    id text not null,
    empresa_id text not null references public.empresas(id) on delete cascade,
    nome text not null,
    descricao text,
...
create index if not exists idx_produtos_menu_empresa_id on public.produtos_menu(empresa_id);

-- 7. Pedidos Cardapio (Customer orders tracker)
...
create index if not exists idx_pedidos_cardapio_empresa_id on public.pedidos_cardapio(empresa_id);
...
-- Create simple public policies for CRUD with tenant-isolation
create policy "Allow all operations for development and demonstration users in empresas"
on public.empresas for all using (true) with check (true);

create policy "Allow all operations for development and demonstration users in utilizadores"
on public.utilizadores for all using (true) with check (true);

create policy "Allow all operations for development and demonstration users in clientes"
on public.clientes for all using (true) with check (true);

create policy "Allow all operations for development and demonstration users in financeiro_metricas"
on public.financeiro_metricas for all using (true) with check (true);

create policy "Allow all operations for development and demonstration users in burger_components"
on public.burger_components for all using (true) with check (true);

create policy "Allow all operations for development and demonstration users in produtos_menu"
on public.produtos_menu for all using (true) with check (true);

create policy "Allow all operations for development and demonstration users in pedidos_cardapio"
on public.pedidos_cardapio for all using (true) with check (true);

-- Insert central admin tenant / base company for demo compatibility
...
-- Setup delicious demo products for BurguersPrime Luanda to serve instantly on the cardapio!
